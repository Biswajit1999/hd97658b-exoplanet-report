...
def test_pipeline_reproduces_committed_statistics():
    analysis.main()
    values = read_summary()
    assert np.isclose(values["depth_ppm"], 757.54674759, rtol=0, atol=0.1)
    assert np.isclose(values["chi_square_flat"], 5147.95073106, rtol=1e-9)
    assert np.isclose(values["chi_square_box"], 3544.25931488, rtol=1e-9)
    assert np.isclose(values["delta_chi_square"], 1603.69141617, rtol=1e-9)
